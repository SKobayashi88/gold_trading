...
gemini_model = st.sidebar.selectbox("CRO Model", ["gemini-3.0-flash", "gemini-2.0-flash-exp", "gemini-1.5-pro"], index=0)
...
# 3. Strategy
with tabs[2]:
    st.subheader("🤖 CIO (Strategy)"); n_s = st.slider("Strategies", 1,3,2)
...
with tabs[5]:
    ...
    st.caption("Calculates optimal position size based on Backtest & Volatility.")
